...
class Solution{
    ...
    // Node* solve(int in[],int pre[],int &index,int inorderStart, int inorderEnd,int size,map<int,int> pos){
    ...
    //     root->left = solve(in,pre,index,inorderStart,position-1,size,pos);
    //     root->right = solve(in,pre,index,position+1,inorderEnd,size,pos);
    ...
    //         pos[in[i]] = i;
    //     }
    //     Node* root= solve(in,pre,index,0,n-1,n,pos);
    //     return root;
